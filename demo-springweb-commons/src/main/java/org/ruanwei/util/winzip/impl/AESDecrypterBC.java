package org.ruanwei.util.winzip.impl;
...
public class AESDecrypterBC extends AESCryptoBase implements AESDecrypter {
	...
	// TODO consider keySize (but: we probably need to adapt the key size for
	// the zip file as well)
	public void init(String pwStr, int keySize, byte[] salt,
			byte[] pwVerification) throws ZipException {
		...
		cipherParameters = generator
				.generateDerivedParameters(KEY_SIZE_BIT * 2 + 16);
		...
		// checksum added to the end of the encrypted data, update on each
		// encryption call
		...
		// incremented on each 16 byte block and used as encryption NONCE
		// (ivBytes)
		nonce = 1;
	}
	...
	protected void decryptBlock(byte[] in, int pos, int length) {
		...
		ParametersWithIV ivParams = new ParametersWithIV(cipherParameters,
				ivBytes);
