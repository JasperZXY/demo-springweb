<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="org.ruanwei.demo.user.entity.User" %>
			...
			<div class="form">
				<form method="post" action="<%=ctx%>/user/doEdit.html">
					...
					<div class="row">
						<label>爱好:</label>
						<div class="el">
							<input type="checkbox" name="hobbyArray" id="hobbyArray1" value="1" <%=((((User)request.getAttribute("user")).getHobby())&1)!=0?"checked=\"checked\"":"" %>/>政治
							<input type="checkbox" name="hobbyArray" id="hobbyArray2" value="2" <%=((((User)request.getAttribute("user")).getHobby())&2)!=0?"checked=\"checked\"":"" %>/>经济
							<input type="checkbox" name="hobbyArray" id="hobbyArray4" value="4" <%=((((User)request.getAttribute("user")).getHobby())&4)!=0?"checked=\"checked\"":"" %>/>文化
							<input type="checkbox" name="hobbyArray" id="hobbyArray8" value="8" <%=((((User)request.getAttribute("user")).getHobby())&8)!=0?"checked=\"checked\"":"" %>/>军事
						</div>
					</div>
					<div class="row">
						<label>个人简介:</label>
